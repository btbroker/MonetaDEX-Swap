#!/usr/bin/env bash
# Smoke test: healthz, /v1/providers, POST /v1/quote (Polygon USDC -> USDT).
...
echo "Smoke: $BASE_URL (healthz, providers, quote USDC->USDT)"
...
if echo "$RESP" | grep -q '"routes":\[\]'; then
  echo "FAIL: POST /v1/quote returned no routes"
...
echo "  POST /v1/quote OK (routes present)"
echo "Smoke OK"
