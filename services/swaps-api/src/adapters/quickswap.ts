...
/**
 * QuickSwap adapter for same-chain swaps
 * Direct integration with QuickSwap protocol
 * Polygon native DEX
 */
export class QuickSwapAdapter extends BaseAdapter {
  name = "quickswap";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from QuickSwap");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% QuickSwap fee
