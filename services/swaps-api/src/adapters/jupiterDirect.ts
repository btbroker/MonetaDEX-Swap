...
/**
 * Jupiter Direct adapter for same-chain swaps
 * Direct integration with Jupiter protocol (not aggregator mode)
 * Solana's largest aggregator, also operates as direct DEX
 */
export class JupiterDirectAdapter extends BaseAdapter {
  name = "jupiter-direct";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Jupiter Direct");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% Jupiter fee
