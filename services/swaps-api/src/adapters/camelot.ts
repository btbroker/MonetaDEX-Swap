...
/**
 * Camelot adapter for same-chain swaps
 * Direct integration with Camelot protocol
 * Arbitrum native DEX
 */
export class CamelotAdapter extends BaseAdapter {
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Camelot");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% Camelot fee
