...
/**
 * Bancor adapter for same-chain swaps
 * Direct integration with Bancor protocol
 * Single-sided liquidity, impermanent loss protection
 */
export class BancorAdapter extends BaseAdapter {
  name = "bancor";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Bancor");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.001; // 0.1% Bancor fee (low)
