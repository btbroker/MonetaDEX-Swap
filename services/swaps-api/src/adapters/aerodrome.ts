...
/**
 * Aerodrome adapter for same-chain swaps
 * Direct integration with Aerodrome protocol
 * Base native DEX, Velodrome fork
 */
export class AerodromeAdapter extends BaseAdapter {
  name = "aerodrome";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Aerodrome");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.0005; // 0.05% Aerodrome fee (very low, like Velodrome)
