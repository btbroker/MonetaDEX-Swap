...
/**
 * Maverick adapter for same-chain swaps
 * Direct integration with Maverick protocol
 * Concentrated liquidity DEX, multiple chains
 */
export class MaverickAdapter extends BaseAdapter {
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Maverick");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% Maverick fee
