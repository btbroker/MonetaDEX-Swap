...
/**
 * Meteora adapter for same-chain swaps
 * Direct integration with Meteora protocol
 * Solana dynamic liquidity pools (DLMM, Standard, Stable, Weighted, Concentrated)
 */
export class MeteoraAdapter extends BaseAdapter {
  name = "meteora";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Meteora");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.0025; // 0.25% Meteora fee
