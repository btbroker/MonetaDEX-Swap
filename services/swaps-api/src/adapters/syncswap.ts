...
/**
 * SyncSwap adapter for same-chain swaps
 * Direct integration with SyncSwap protocol
 * zkSync native DEX
 */
export class SyncSwapAdapter extends BaseAdapter {
  name = "syncswap";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from SyncSwap");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% SyncSwap fee
    ...
    const route: Omit<Route, "routeId"> = {
      ...
      estimatedGas: "120000", // zkSync has lower gas
