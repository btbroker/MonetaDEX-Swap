...
/**
 * Phoenix adapter for same-chain swaps
 * Direct integration with Phoenix protocol
 * Solana order book DEX
 */
export class PhoenixAdapter extends BaseAdapter {
  name = "phoenix";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Phoenix");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.0004; // 0.04% Phoenix fee (very low for order book)
    ...
    const route: Omit<Route, "routeId"> = {
      ...
      priceImpactBps: 5, // Lower impact for order book
      steps: [{
        type: "swap",
        provider: this.name,
        fromChainId: request.fromChainId,
