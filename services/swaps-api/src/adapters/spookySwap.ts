...
/**
 * SpookySwap adapter for same-chain swaps
 * Direct integration with SpookySwap protocol
 * Fantom native DEX
 */
export class SpookySwapAdapter extends BaseAdapter {
  name = "spookyswap";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from SpookySwap");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% SpookySwap fee
