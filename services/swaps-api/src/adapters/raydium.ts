...
/**
 * Raydium adapter for same-chain swaps
 * Direct integration with Raydium protocol
 * Solana AMM with high liquidity
 */
export class RaydiumAdapter extends BaseAdapter {
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Raydium");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.0025; // 0.25% Raydium fee
