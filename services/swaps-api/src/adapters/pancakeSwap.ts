...
/**
 * PancakeSwap adapter for same-chain swaps
 * Direct integration with PancakeSwap protocol
 * Top DEX on BSC with $800M+ daily volume
 */
export class PancakeSwapAdapter extends BaseAdapter {
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from PancakeSwap");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.0025; // 0.25% PancakeSwap fee
  ...
  async getTx(routeId: string, request: TxRequest): Promise<TxResponse> {
    ...
    return {
      routeId,
      txData: "0x",
      to: "0x0000000000000000000000000000000000000000", // TODO: PancakeSwap router address
      value: "0",
      gasLimit: "180000",
