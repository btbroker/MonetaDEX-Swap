...
/**
 * TraderJoe adapter for same-chain swaps
 * Direct integration with TraderJoe protocol
 * Multi-chain DEX, strong on Avalanche
 */
export class TraderJoeAdapter extends BaseAdapter {
  name = "traderjoe";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from TraderJoe");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% TraderJoe fee
