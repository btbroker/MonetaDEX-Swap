...
/**
 * Velodrome adapter for same-chain swaps
 * Direct integration with Velodrome protocol
 * Optimism native DEX with ve(3,3) model
 */
export class VelodromeAdapter extends BaseAdapter {
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Velodrome");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.0005; // 0.05% Velodrome fee (very low)
