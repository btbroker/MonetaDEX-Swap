...
/**
 * Orca adapter for same-chain swaps
 * Direct integration with Orca protocol
 * Solana's top DEX with $1.5B+ daily volume
 */
export class OrcaAdapter extends BaseAdapter {
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Orca");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% Orca fee
    ...
    const route: Omit<Route, "routeId"> = {
      ...
      estimatedGas: "0", // Solana uses compute units, not gas
  ...
  async getTx(routeId: string, request: TxRequest): Promise<TxResponse> {
    ...
    return {
      ...
      value: "0",
      gasLimit: "0", // Solana uses compute units
      gasPrice: "0",
