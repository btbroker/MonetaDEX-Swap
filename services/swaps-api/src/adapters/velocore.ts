...
/**
 * Velocore adapter for same-chain swaps
 * Direct integration with Velocore protocol
 * zkSync Era native DEX
 */
export class VelocoreAdapter extends BaseAdapter {
  name = "velocore";
  ...
  async getQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    try {
      ...
      logger.error({ error, provider: this.name }, "Failed to get quote from Velocore");
      return [];
    }
  }
  ...
  private async getMockQuote(request: QuoteRequest): Promise<Route[]> {
    ...
    const dexFeeRate = 0.003; // 0.3% Velocore fee
    ...
    const route: Omit<Route, "routeId"> = {
      ...
      estimatedGas: "120000", // zkSync Era has lower gas
